...
async def on_message(message):
	...
	if game and game.in_progress:
		game.score(message.author.name)
...
async def game_handler(ctx):
	...
	while game.in_progress:
		...
		song = game.new_round()['song']
		song_search_string = f"{song['artists'][0]} {song['name']} audio"

		async with ctx.typing():
			player = await YTDLSource.from_url(song_search_string, loop=bot.loop, stream=True)
			voice_channel.play(player, after=lambda e: print(f'Player error: {e}') if e else None)

		message = discord.Embed()
		message.add_field(name='Song', value=song['name'])
		for artist in song['artists']:
			message.add_field(name='Artist', value=artist)
